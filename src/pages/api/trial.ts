...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    await transporter.sendMail({
      ...
      replyTo: email,
      subject: `Demo Access — ${name || email}`,
      html: `
        <div style="font-family:sans-serif;max-width:500px;margin:0 auto;color:#0f1c2e;">
          <div style="background:#0f1c2e;padding:24px 32px;border-radius:12px 12px 0 0;">
            <h1 style="color:#1ecfad;margin:0;font-size:20px;">Demo Access Request</h1>
            <p style="color:rgba(255,255,255,0.5);margin:4px 0 0;font-size:13px;">via Product Showcase gate</p>
          </div>
          <div style="border:1px solid #e2e8f0;border-top:none;padding:32px;border-radius:0 0 12px 12px;font-size:14px;">
            <p style="margin:0;">A visitor unlocked the interactive demo.</p>
            ${name ? `<p style="margin:16px 0 0;"><strong>Name:</strong> ${name}</p>` : ''}
            <p style="margin:8px 0 0;"><strong>Email:</strong> <a href="mailto:${email}" style="color:#1ecfad;">${email}</a></p>
          </div>
        </div>
      `,
    });
